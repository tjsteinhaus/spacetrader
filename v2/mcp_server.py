...
# ── Constants — auto-loaded from DB (or defaults if fresh install) ────────────
_cfg_row: dict = {}
try:
    with db._conn() as _c:
        _wp_row = _c.execute(
        ...
        if _wp_row:
            _sys = _wp_row[0]
            _ac_row = _c.execute(
                "SELECT key, value FROM agent_config WHERE callsign = (SELECT callsign FROM agent_config LIMIT 1)"
            ).fetchall()
            _cfg_row = {r[0]: r[1] for r in _ac_row}
except Exception:
    pass

SYSTEM        = _cfg_row.get("SYSTEM",       "X1-HU91")
COMMAND_SHIP  = _cfg_row.get("COMMAND_SHIP",  "TYLERMASTERY-1")
SHIPYARD_WP   = _cfg_row.get("SHIPYARD_WP",   "X1-HU91-H52")
HQ_WP         = _cfg_row.get("FACTION_HQ_WP", "X1-HU91-A1")
ASTEROID      = _cfg_row.get("ASTEROID",       "X1-HU91-B8")
