"""
discord_notify.py — Fire-and-forget Discord webhook notifications for v2 bot.

Port of ../discord_notify.py adapted for use within the v2/ package.
All call sites are non-blocking (daemon thread per send).

Webhook URL is read from the 'discord_webhook' bot_setting in the DB,
or the DISCORD_WEBHOOK environment variable. If neither is set, all
...
Usage:
    ...
    discord.send_scrap(ship, received)
...
def send_status(
    ...
    _post({"embeds": [_embed(
        ...
        footer=f"SpaceTraders v2 • {label}",
    )]})
...
def send_trade_start(
    ...
    _post({"embeds": [_embed(
        ...
        footer=f"SpaceTraders v2 • {_now_str()}",
    )]})
